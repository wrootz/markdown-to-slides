    ...
    for (const slideMd of slidesContent) {
      ...
      requests.push({
        createSlide: {
          ...
          slideProperties: {
            // Usa o layout padrão "TITLE_AND_BODY".
            // Para "padrão do Google Presentation" real, idealmente você obteria
            // os IDs de layout existentes da apresentação e usaria os placeholders.
            // Para simplificar, estamos criando caixas de texto com posições fixas.
      ...
      requests.push({
        createShape: {
          ...
          elementProperties: {
            ...
            transform: { // Posição e tamanho iniciais da caixa de texto (em PT - pontos)
              scaleX: 1, scaleY: 1,
              translateX: 50,
              translateY: 50,
              unit: 'PT'
      ...
      requests.push({
        insertText: {
          objectId: titleShapeObjectId,
          text: slideTitle || 'Slide ' + (slideCount + 1) // Usa o título extraído ou um título padrão
        }
      });
      ...
      requests.push({
        updateTextStyle: {
            objectId: titleShapeObjectId,
            textRange: { type: 'ALL' }, // Aplica o estilo a todo o texto na caixa
            style: {
                fontSize: { magnitude: 24, unit: 'PT' }, // Tamanho de fonte 24pt
                bold: true // Texto em negrito
            },
            fields: 'fontSize,bold' // Especifica quais campos do estilo estão sendo atualizados
        }
      });
      ...
      requests.push({
        createShape: {
          ...
          elementProperties: {
            ...
            transform: { // Posição da caixa de corpo, abaixo do título
              scaleX: 1, scaleY: 1,
              translateX: 50,
              translateY: 120,
              unit: 'PT'
      ...
      requests.push({
        insertText: {
          objectId: bodyShapeObjectId,
          text: slideBodyElements.join('\n\n') // Junta os elementos do corpo com duas quebras de linha para criar parágrafos separados
        }
      });
      ...
      requests.push({
        updateTextStyle: {
            ...
            textRange: { type: 'ALL' },
            style: {
                fontSize: { magnitude: 14, unit: 'PT' } // Tamanho de fonte 14pt
            },
            fields: 'fontSize'
        }
      });

      slideCount++; // Incrementa o contador de slides para o próximo loop
    }
